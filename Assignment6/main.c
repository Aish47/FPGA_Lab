// Assigment 6 to display truth tabel
...
//Set GPIO direction
    ...
    PyHal_GPIO_SetDir(23,0); // MSB : A
    PyHal_GPIO_SetDir(25,0);
    PyHal_GPIO_SetDir(27,0); // LSB : C 
    ...
    while(1)
    {
        ...
        lhs = a&((!a)|b)&c&(a|b); //LHS
        if(out == 1)
    	sevenseg(1,0,0,1,1,1,1);//The number 1 on seven segment(RHS Output)
    	else
    	sevenseg(0,0,0,0,0,0,1);//The number 0 (RHS Output)
    	if(lhs == 1)
    	PyHal_GPIO_Set(22,1); // number 1 on Red LED (LHS Output)
    	else
    	PyHal_GPIO_Set(22,0); // number 0 (RHS Output)
    }      
